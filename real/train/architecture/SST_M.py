...
class SpatialAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                SwinTransformerBlock(dim=dim, input_resolution=size // (2 ** stage),
...
class SST_block(nn.Module):
    def __init__(self, dim=28, size=384):
        ...
        """spatial"""
        # Encoder_spatial
        self.down_0_0 = SpatialAB(stage=0, dim=28, num_blocks=1, size=size)
        self.downTranspose_0_0 = nn.Conv2d(28, 56, 4, 2, 1, bias=False)


        self.down_1_0 = SpatialAB(stage=1, dim=56, num_blocks=1, size=size)
        self.downTranspose_1_0 = nn.Conv2d(56, 112, 4, 2, 1, bias=False)

        # Bottleneck_spatial
        self.bottleneck_2_0 = SpatialAB(stage=2, dim=112, num_blocks=1, size=size)
        ...
        self.up_1_1 = SpatialAB(stage=1, dim=56, num_blocks=1, size=size)
        ...
        self.up_0_1 = SpatialAB(stage=1, dim=28, num_blocks=1, size=size)
        ...
        self.up_0_2 = SpatialAB(stage=1, dim=28, num_blocks=1, size=size)
...
class SST_M(nn.Module):
    def __init__(self, in_channels=28, out_channels=28, n_feat=28, stage=1, size=496):
        super(SST_M, self).__init__()

        self.fution = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.fution0 = nn.Conv2d(28, 28, 1, padding=0, bias=True)
        # 1-stg
        self.conv_in1 = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=True)
        modules_body1 = [SST_block(dim=n_feat, size=size) for _ in range(stage)]
        self.fution1 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body1 = nn.Sequential(*modules_body1)
        self.conv_out1 = nn.Conv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2, bias=True)

        # 2-stg
        self.conv_in2 = nn.Conv2d(in_channels, n_feat, kernel_size=5, padding=(5 - 1) // 2, bias=True)
        modules_body2 = [SST_block(dim=n_feat, size=size) for _ in range(stage)]
        self.fution2 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body2 = nn.Sequential(*modules_body2)
        self.conv_out2 = nn.Conv2d(n_feat, out_channels, kernel_size=5, padding=(5 - 1) // 2, bias=True)
    ...
    def initial_x(self, y, input_mask=None):
        ...
        if input_mask is None:
            ...
            x = self.fution(torch.cat([x, input_mask], dim=1))
        return x
    ...
    def forward(self, y, mask=None):
        ...
        """
        if mask==None:
            mask = torch.rand((1,28,512,512)).cuda()

        x = self.initial_x(y, mask)
        ...
        x = y - input_meas
        x = shift_back(x)
        x = self.fution2(torch.cat([x, mask], dim=1))
        x = self.conv_in2(x)
        h9 = self.body2(x)
