...
class SST_block_spatial(nn.Module):
    def __init__(self, dim=28):
        super(SST_block_spatial, self).__init__()
        # Input projection
...
class SST_block_spectral(nn.Module):
    def __init__(self, dim=28):
        super(SST_block_spectral, self).__init__()
        # Input projection
        self.embedding = nn.Conv2d(28, 28, 3, 1, 1, bias=False)
        ...
        self.conv_0_5 = nn.Conv2d(56, 28, 1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        """spectral"""
        # down
        x_0_3 = self.down_0_3(fea)
        fea = self.downTranspose_0_3(x_0_3)

        x_1_2 = self.down_1_2(fea)
        ...
        x_0_5 = self.conv_0_5(torch.cat([x_0_5, x_0_3], dim=1))
...
class SST_S(nn.Module):
    def __init__(self, in_channels=28, out_channels=28, n_feat=28, stage=1):
        super(SST_S, self).__init__()
        self.fution = nn.Conv2d(28, 28, 1, padding=0, bias=True)

        self.conv_in1 = nn.Conv2d(in_channels, n_feat, kernel_size=5, padding=(5 - 1) // 2, bias=False)
        modules_body1 = [SST_block_spatial(dim=n_feat) for _ in range(stage)]
        self.fution1 = nn.Conv2d(84, 28, 1, padding=0, bias=True)
        self.body1 = nn.Sequential(*modules_body1)
        self.conv_out1 = nn.Conv2d(n_feat, out_channels, kernel_size=5, padding=(5 - 1) // 2, bias=False)


        self.conv_in2 = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=False)
        modules_body2 = [SST_block_spectral(dim=n_feat) for _ in range(stage)]
        self.fution2 = nn.Conv2d(84, 28, 1, padding=0, bias=True)
        self.body2 = nn.Sequential(*modules_body2)
        self.conv_out2 = nn.Conv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2, bias=False)

    def forward(self, y, mask=None, shiftmask=None):
        """
        x: [b,c,h,w]
        return out:[b,c,h,w]
        """
        if mask==None:
            mask = torch.rand((1,28,384,384)).cuda()
        if shiftmask==None:
            shiftmask = torch.rand((1,28,384,384)).cuda()

        x = self.fution1(torch.cat([y, mask, shiftmask], dim=1))
        ...
        x = y - input_meas
        x = self.fution2(torch.cat([x, mask, shiftmask], dim=1))
        x = self.conv_in2(x)
        h2 = self.body2(x)
