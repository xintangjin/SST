...
class SpatialAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                SwinTransformerBlock(dim=dim, input_resolution=384 // (2 ** stage),
...
class SST_LPlus(nn.Module):
    def __init__(self, in_channels=28, out_channels=28, n_feat=28, stage=1):
        ...
        # 3-stg
        self.conv_in3 = nn.Conv2d(in_channels, n_feat, kernel_size=7, padding=(7 - 1) // 2, bias=False)
        modules_body3 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution3 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body3 = nn.Sequential(*modules_body3)
        self.conv_out3 = nn.Conv2d(n_feat, out_channels, kernel_size=7, padding=(7 - 1) // 2, bias=False)

        # 4-stg
        self.conv_in4 = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=False)
        modules_body4 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution4 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body4 = nn.Sequential(*modules_body4)
        self.conv_out4 = nn.Conv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2, bias=False)

        # 5-stg
        self.conv_in5 = nn.Conv2d(in_channels, n_feat, kernel_size=5, padding=(5 - 1) // 2, bias=False)
        modules_body5 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution5 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body5 = nn.Sequential(*modules_body5)
        self.conv_out5 = nn.Conv2d(n_feat, out_channels, kernel_size=5, padding=(5 - 1) // 2, bias=False)

        # 6-stg
        self.conv_in6 = nn.Conv2d(in_channels, n_feat, kernel_size=7, padding=(7 - 1) // 2, bias=False)
        modules_body6 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution6 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body6 = nn.Sequential(*modules_body6)
        self.conv_out6 = nn.Conv2d(n_feat, out_channels, kernel_size=7, padding=(7 - 1) // 2, bias=False)

        # 7-stg
        self.conv_in7 = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2, bias=False)
        modules_body7 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution7 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body7 = nn.Sequential(*modules_body7)
        self.conv_out7 = nn.Conv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2, bias=False)

        # 8-stg
        self.conv_in8 = nn.Conv2d(in_channels, n_feat, kernel_size=5, padding=(5 - 1) // 2, bias=False)
        modules_body8 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution8 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body8 = nn.Sequential(*modules_body8)
        self.conv_out8 = nn.Conv2d(n_feat, out_channels, kernel_size=5, padding=(5 - 1) // 2, bias=False)

        # 9-stg
        self.conv_in9 = nn.Conv2d(in_channels, n_feat, kernel_size=7, padding=(7 - 1) // 2, bias=False)
        modules_body9 = [SST_block(dim=n_feat) for _ in range(stage)]
        self.fution9 = nn.Conv2d(56, 28, 1, padding=0, bias=True)
        self.body9 = nn.Sequential(*modules_body9)
        self.conv_out9 = nn.Conv2d(n_feat, out_channels, kernel_size=7, padding=(7 - 1) // 2, bias=False)
    ...
    def forward(self, y, Phi=None, mask=None):
        ...
        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution3(torch.cat([x, Phi], dim=1))
        x = self.conv_in3(x)
        h9 = self.body3(x)
        h9 = self.conv_out3(h9)
        h9 = h9 + x
        h = h + h9

        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution4(torch.cat([x, Phi], dim=1))
        x = self.conv_in4(x)
        h9 = self.body4(x)
        h9 = self.conv_out4(h9)
        h9 = h9 + x
        h = h + h9

        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution5(torch.cat([x, Phi], dim=1))
        x = self.conv_in5(x)
        h9 = self.body5(x)
        h9 = self.conv_out5(h9)
        h9 = h9 + x
        h = h + h9

        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution6(torch.cat([x, Phi], dim=1))
        x = self.conv_in6(x)
        h9 = self.body6(x)
        h9 = self.conv_out6(h9)
        h9 = h9 + x
        h = h + h9

        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution7(torch.cat([x, Phi], dim=1))
        x = self.conv_in7(x)
        h9 = self.body7(x)
        h9 = self.conv_out7(h9)
        h9 = h9 + x
        h = h + h9
        ...
        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution8(torch.cat([x, Phi], dim=1))
        x = self.conv_in8(x)
        h9 = self.body8(x)
        h9 = self.conv_out8(h9)
        h9 = h9 + x
        h = h + h9

        input_meas = gen_meas_torch(h, mask)
        x = y - input_meas
        x = self.fution9(torch.cat([x, Phi], dim=1))
        x = self.conv_in9(x)
        h9 = self.body9(x)
        h9 = self.conv_out9(h9)
        h9 = h9 + x
        h = h + h9
